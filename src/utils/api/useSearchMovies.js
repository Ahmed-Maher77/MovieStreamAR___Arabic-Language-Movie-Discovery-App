...
function useSearchMovies(movieName) {
	...
	const fetchMovies = async () => {
		try {
			const res = await axios.get(`https://api.themoviedb.org/3/search/movie?api_key=${apikey}&query=${encodeURIComponent(movieName)}&language=${langParam}`);
			return res.data;
		} catch (error) {
			throw new Error(
	...
	const { data, isLoading, error } = useQuery({
		...
		queryFn: fetchMovies,
		enabled: !!movieName, // Only run query when there's a search term
